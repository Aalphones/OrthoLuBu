...
function bornholm_scripts_styles() {
    ...
    wp_enqueue_script('bornholm-menu', get_template_directory_uri() . '/js/compressed.js', array(), false, true);

    /*
     * Loads our main stylesheet.
     */
    wp_enqueue_style('main-style', get_template_directory_uri() . '/css/compressed.css', array(), null);
}
 ...
 * @return string Formatted output in HTML
 */
function the_post_header($heading, $post) {
...
function bornholm_gallery_image_number($images) {
    if ($images) {
        ...
        <p>
            <em><?php
        printf(_n('This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'bornholm'), 'href="' . esc_url(get_permalink())
...
function bornholm_alternative_front_page_more_link($cat) {
    ...
    <article>
    <?php
    $images_child = bornholm_get_gallery_images($post->ID);
...
function bornholm_get_galleries_from_category($cat, $exclude_id, $number_of_galleries, $heading, $title, $show_child_category_hierarchy) {
    ...
    if ($galleries) {
        ...
        <?php } ?>
        <?php
        bornholm_loop_galleries_from_category($galleries, $total_galleries, $number_of_galleries, $gallery_counter, $cat);
    ...
    switch ($comment->comment_type) :
        ...
        default :
            ...
            <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                ...
                <?php
                printf('<a href="%1$s"><time datetime="%2$s">%3$s</time></a>', esc_url(get_comment_link($comment->comment_ID)), get_comment_time('c'), sprintf(_x('%1$s @ %2$s', '1=date 2=time', 'bornholm'), get_comment_date(), get_comment_time())
                );
                ?>
                    </header>
